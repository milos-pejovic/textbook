...
foreach($content['data']['items'][$i]['input-fields'] as $fieldIndex => $fieldData) {
    ...
    if ($isExample){
        ...
        $inputFieldHtml = '<div class="single-input-field"><input type="text" class="question-input example" data-exercise-id="'.$post_id.'" data-answers="' . $answers . '" value="'.$answers.'" readonly="readonly" //><i class="fas fa-check"></i><i class="fas fa-times"></i></div>';
        $questionHtml = str_replace($fieldIndex, $inputFieldHtml, $questionHtml);
    } else {
        $answers = implode(" | ",$fieldData['answers']);
        $inputFieldHtml = '<div class="single-input-field"><input type="text" class="question-input" data-exercise-id="'.$post_id.'" data-answers="' . $answers . '" //><i class="fas fa-check"></i><i class="fas fa-times"></i></div>';
        $questionHtml = str_replace($fieldIndex, $inputFieldHtml, $questionHtml);
    }
